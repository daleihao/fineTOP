function [dif_variables] = get_variable_all_nolw(variable_name_new)
...
for season_i = [5 1:4]
    ...
    elseif(strcmp(variable_name_new, 'Rnet'))
        ...
        load(['data/' variable_name_new '_seasonal_ELM_only_' num2str(season_i) '_with_top.mat']);

        if (strcmp(variable_name_new, 't_rad_grc'))
            kTOP_nolw_nosurf_seasons = kTOP_nolw_nosurf_seasons .* sqrt(sqrt(cos(slope*pi/180))); %.* sqrt(sqrt(cos(slope*pi/180)));
