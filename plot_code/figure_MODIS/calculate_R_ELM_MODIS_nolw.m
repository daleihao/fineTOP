...
for gridSize = [10]
    ...
    for variable_name_i = 2:3
        variable_name = variable_names{variable_name_i};
        for season_i = 1:5

            load([ '../data/' variable_name '_seasonal_ELM_MODIS_' num2str(season_i) '_modify_with_top.mat']);
            ...
            for i = 1:numGridsRow
                for j = 1:numGridsCol
                    ...
                    slopes(i,j) = mean(slope_ij(:),'omitnan');
                    % Example: Calculate mean value for each grid
                    values1 = kTOP_nolw_nosurf_seasons((i-1)*gridSize+1:i*gridSize, (j-1)*gridSize+1:j*gridSize); % You can replace this with any statistical measure
            ...
            save(['R_' variable_name '_' num2str(gridSize) num2str(season_i) '_modify_top.mat'],'R1','R2',"R3","R4","R5");
...
%             imagesc(R4-R5, [-0.2 0.2])
    ...
    save(['R_all' num2str(gridSize) '_modify_top.mat'],'R_aspects','R_MODISs',"Bias_MODISs","RMSE_MODISs","slopes");
end