...
positions = [
    0.09, 0.12, 0.4, 0.85;  % Top subplot
    0.59, 0.12, 0.4, 0.85;  % Middle subplot
    ];
...
variable_names = {'Daytime \Delta\it{T}_{sur} (K)','Nighttime \Delta\it{T}_{sur} (K)','\Delta\it{F}_{sen} (W/m^2)','\Delta\it{F}_{lat} (W/m^2)','\Delta\it{f}_{sno}','\DeltaSWE (mm)'};
for variable_i = 1:2
    ...
    if variable_i <0 || variable_i == 4 || variable_i == 5
        set(gca,'XTickLabel',[])
    end
    set(gca,'FontSize',10);
    ...
    text(5.1,ylims(variable_i,1)*0.85,['(' labels{variable_i} ')'],'fontsize',20,'FontWeight','bold');
end


set(gcf, 'Color', 'w');
export_fig('../figure/Figure_daytime_nighttime_Tsur.pdf', '-pdf', '-m2', '-painters','-nocrop');
