...
PFTs = ones(size(LC_ELM_Data))*4;
PFTs(LC_ELM_Data>=1 & LC_ELM_Data<=8) = 1;
PFTs(LC_ELM_Data>=9 & LC_ELM_Data<=11) = 2;
PFTs(LC_ELM_Data>=12 & LC_ELM_Data<=14) = 3;
...
clr.AxisLocation = "in";
clr.Ticks = [1+1/3 2+1/6 3-1/6 3+1/2];
clr.TickLabels = {'Forest','Shrub','Grass','Others'};
...
set(gcf, 'Color', 'w');
export_fig('../figure/Figure_2_study_area.pdf', '-pdf', '-m2', '-painters','-nocrop');
