...
variable_names = {'\it{SW}_{dir} (W/m^2)','\it{SW}_{sky} (W/m^2)','\it{SW}_{adj} (W/m^2)','\it{LW}_{sky} (W/m^2)','\it{LW}_{adj} (W/m^2)','Albedo'};
for variable_i = 1:6
    ...
    h = daboxplot(data,'groups',aspect_NS, 'outsymbol','kx',...
     ...
     if variable_i == 2
         legend([h.bx(1,:)],{'North Slope','South Slope'}, 'Orientation', 'horizontal', 'Location', 'north', 'Box', 'off');    % add the legend manually
     end
...
set(gcf, 'Color', 'w');
export_fig('../figure/Figure_seperate_albedo_SN.pdf', '-pdf', '-m2', '-painters','-nocrop');
