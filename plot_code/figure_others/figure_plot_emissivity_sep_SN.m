...
load('vegetation_emiss.mat');

EVF_difs = nan(900*700,5);
tmp = nanmean(EVF_dif(:,:,[12 1 2]),3);
EVI_difs(:,1) = tmp(:);
tmp = nanmean(EVF_dif(:,:,[3 4 5]),3);
EVI_difs(:,2) = tmp(:);

tmp = nanmean(EVF_dif(:,:,[6 7 8]),3);
EVI_difs(:,3) = tmp(:);

tmp = nanmean(EVF_dif(:,:,[9 10 11]),3)
EVI_difs(:,4) = tmp(:);

tmp = nanmean(EVF_dif(:,:,[1:12]),3);
EVI_difs(:,5) = tmp(:);
...
variable_names = {'\Delta\it{\epsilon}_{veg}','\Delta\it{SW}_{sky} (W/m^2)','\Delta\it{SW}_{adj} (W/m^2)','\Delta\it{LW}_{sky} (W/m^2)','\Delta\it{LW}_{adj} (W/m^2)','\Delta\it{A}_{sur}'};
for variable_i = 1:1
...
set(gcf, 'Color', 'w');
export_fig('../figure/Figure_veg_emiss_SN.pdf', '-pdf', '-m2', '-painters','-nocrop');
