...
FSH_kTOPs = kTOP_nolw_nosurf_seasons;
...
load(['data/' variable_name '_seasonal_ELM_only_' num2str(season_i) '_with_top.mat']);
EFLX_LH_TOT_kTOPs = kTOP_nolw_nosurf_seasons;
...
variable_name = 'TV';
load(['data/' variable_name '_seasonal_ELM_only_' num2str(season_i) '_with_top.mat']);
TV_kTOPs = kTOP_nolw_nosurf_seasons;
...
kTOP_surf_seasons = kTOP_nolw_nosurf_seasons;% .* sqrt(sqrt(cos(slope*pi/180)));
...
variable_name = 'FSNO';
load(['data/' variable_name '_seasonal_ELM_only_' num2str(season_i) '_with_top.mat']);
FSNO_kTOPs = kTOP_nolw_nosurf_seasons;
...
load(['data/' variable_name '_seasonal_ELM_only_' num2str(season_i) '_with_top.mat']);
H2OSNO_kTOPs = kTOP_nolw_nosurf_seasons;
...
set(gca,'XTickLabel',[])
insetPos = [0.07 + (1-1)*0.31 0.55 0.1 0.13]; % Adjust as needed
...
insetPos = [0.07 + (2-1)*0.31 0.55 0.1 0.13]; % Adjust as needed
insetAxes = axes('Position', insetPos);
...
insetPos = [0.07 + (3-1)*0.31 0.55 0.1 0.13]; % Adjust as needed
insetAxes = axes('Position', insetPos);
...
insetPos = [0.07 + (1-1)*0.31 0.06 0.1 0.13]; % Adjust as needed
insetAxes = axes('Position', insetPos);
...
insetPos = [0.07 + (2-1)*0.31 0.06 0.1 0.13]; % Adjust as needed
insetAxes = axes('Position', insetPos);
...
insetPos = [0.07 + (3-1)*0.31 0.06 0.1 0.13]; % Adjust as needed
insetAxes = axes('Position', insetPos);
...
set(gcf, 'Color', 'w');
export_fig('../figure/Figure_energy_spatial_nolw.pdf', '-pdf', '-m2', '-painters','-nocrop');
