...
plot_polar_plot(5,'FSDS',60,'(a) \Delta\it{SW}_{down}')

s=subplot('position', [0.05 + (2-1)*0.31 0.52 0.3 0.4]);
plot_polar_plot(5,'FSA',60,'(b) \Delta\it{SW}_{net}')

s=subplot('position', [0.05 + (3-1)*0.31 0.52 0.3 0.4]);
plot_polar_plot(5,'FLDS',60,'(c) \Delta\it{LW}_{down}')

s=subplot('position', [0.05 + (1-1)*0.31 0.05 0.3 0.4]);
plot_polar_plot(5,'FIRA',60,'(d) \Delta\it{LW}_{net}')

s=subplot('position', [0.05 + (2-1)*0.31 0.05 0.3 0.4]);
plot_polar_plot(5,'Rnet',60,'(e) \Delta\it{R}_{net}')
...
set(gcf, 'Color', 'w');
export_fig('../figure/Figure_4_radiation_aspect.pdf', '-pdf', '-m2', '-painters','-nocrop');
